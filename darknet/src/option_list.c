...
linked_list *read_data_cfg(char *filename)
{
...
{
    metadata m = {0};
	linked_list *options = read_data_cfg(file);
...
char *option_find_str(linked_list *l, char *key, char *def)
{
...
int option_find_int(linked_list *l, char *key, int def)
{
    char *v = option_find(l, key);
    if(v) return atoi(v);
...
float option_find_float(linked_list *l, char *key, float def)
{
    char *v = option_find(l, key);
    if(v) return atof(v);
